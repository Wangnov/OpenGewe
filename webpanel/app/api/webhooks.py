...
async def receive_webhook(
    ...
    try:
        ...
        if bot:
            ...
            if not bot:
                logger.warning(f"未找到机器人: schema_id={bot_schema_id}, app_id={payload.Appid}")
                raise HTTPException(
        ...
        from_wxid = data.get("FromUserName", {}).get("string") if data.get("FromUserName") else None
        to_wxid = data.get("ToUserName", {}).get("string") if data.get("ToUserName") else None
        ...
        async with get_bot_session(bot_wxid) as bot_session:
            ...
            logger.info(f"Webhook消息已存储: bot={bot_wxid}, type={payload.TypeName}, msg_id={msg_id}")
...
async def test_webhook(
    ...
    try:
        ...
        async with get_bot_session(bot_wxid) as bot_session:
            # 查询最近5条消息
            stmt = select(RawCallbackLog).where(
                RawCallbackLog.bot_wxid == bot_wxid
            ).order_by(RawCallbackLog.received_at.desc()).limit(5)
    ...
    try:
        ...
        async with get_bot_session(bot_wxid) as bot_session:
            ...
            stmt = select(RawCallbackLog).where(
                and_(
                    RawCallbackLog.bot_wxid == bot_wxid,
                    RawCallbackLog.processed == False
                )
            ).order_by(RawCallbackLog.received_at.asc()).limit(limit)
