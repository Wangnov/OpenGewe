...
from ..schemas.auth import LoginRequest, LoginResponse, RefreshTokenRequest, ChangePasswordRequest
...
async def login(
    ...
    if not user:
        ...
        await _log_login_attempt(
            session, None, client_ip, user_agent, 
            LoginStatus.FAILED, "用户不存在"
        )
        raise HTTPException(
    ...
    if not user.is_active:
        await _log_login_attempt(
            session, user.id, client_ip, user_agent,
            LoginStatus.FAILED, "用户已被禁用"
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="账户已被禁用"
...
async def change_password(
    ...
    # 验证原密码
    if not security_manager.verify_password(password_data.old_password, user.hashed_password):
        raise HTTPException(
