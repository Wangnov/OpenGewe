...
class DatabaseManager:
    ...
    async def create_bot_schema(self, bot_wxid: str) -> str:
        ...
        async with self._session_makers["admin_data"]() as session:
            try:
                # 创建数据库Schema
                await session.execute(text(f"CREATE DATABASE IF NOT EXISTS {schema_name}"))
...
async def get_db_session(schema_name: str = "admin_data") -> AsyncGenerator[AsyncSession, None]:
    """获取数据库会话（依赖注入用）"""
