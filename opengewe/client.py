...
class GeweClient:
    ...
    async def start_login(self) -> None:
        ...
        if device_list_success:
            ...
            if self.app_id not in device_list_result and self.app_id != "":
                print(
                    f'❌ 传入的 app_id: {self.app_id} 不在已登录设备的列表中\n   请传入正确的 app_id。如需登录新设备，请传入 app_id = ""'
                )
                return
        else:
            if device_list_result.get(
                "ret"
            ) == 500 and "不可用或已过期" in device_list_result.get("msg", ""):
                print(
                    f"⚠️ 设置的 token: {self.token} 已过期或不可用，即将重新获取 token..."
                )
            elif device_list_result.get(
                "ret"
            ) == 500 and "header:X-GEWE-TOKEN 不可为空" in device_list_result.get(
                "msg", ""
            ):
        ...
        if qrcode_success:
            ...
            if qrcode_result.get("ret") == 500:
                if qrcode_result.get("msg") == "微信已登录，请勿重复登录。":
                    ...
                    qrcode_result.get("data", {}).get("msg")
                    == "已达到最大客户端数量操作"
                ):
                    print(
